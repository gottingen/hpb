...
namespace hpbc {
    ...
    const char* HSChpb::kEnumsInit = "enums_layout";
    const char* HSChpb::kExtensionsInit = "extensions_layout";
    const char* HSChpb::kMessagesInit = "messages_layout";

    void HSChpb::GenerateFile(const DefPoolPair& pools, hpb::FileDefPtr file,Plugin* plugin) {
        Output h_output;
    ...
    void HSChpb::WriteHeader(const DefPoolPair& pools, hpb::FileDefPtr file,Output& output) {
        ...
        for (auto ext : this_file_exts) {
            GenerateExtensionDeclare(pools, ext, output);
        }
    ...
    void HSChpb::ForwardDeclareMiniTableInit(hpb::MessageDefPtr message, Output& output) const {
        if (bootstrap) {
    ...
    std::string HSChpb::MessageInitName(hpb::MessageDefPtr descriptor) const {
        return absl::StrCat(MessageName(descriptor), "_msg_init");
    }

    std::string HSChpb::ExtensionLayout(hpb::FieldDefPtr ext) const {
        return absl::StrCat(ExtensionIdentBase(ext), "_", ext.name(), "_ext");
    }

    std::string HSChpb::ExtensionIdentBase(hpb::FieldDefPtr ext) const {
    ...
    void HSChpb::DumpEnumValues(hpb::EnumDefPtr desc, Output& output) const {
    ...
    std::string HSChpb::EnumValueSymbol(hpb::EnumValDefPtr value) const {
        return ToCIdent(value.full_name());
    }

    void HSChpb::GenerateMessageInHeader(hpb::MessageDefPtr message,
        ...
        for (int i = 0; i < message.real_oneof_count(); i++) {
            GenerateOneofDeclare(message.oneof(i), pools, msg_name, output);
        }

        auto field_names = CreateFieldNameMap(message);
        for (auto field : FieldNumberOrder(message)) {
            GenerateClearDeclare(field, pools, msg_name, field_names, output);
            GenerateGettersDeclare(field, pools, msg_name, field_names, output);
            GenerateHazzerDeclare(field, pools, msg_name, field_names, output);
        }

        output("\n");

        for (auto field : FieldNumberOrder(message)) {
            GenerateSettersDeclare(field, pools, msg_name, field_names, output);
        }

        output("\n");
    }

    void HSChpb::GenerateMessageInSource(hpb::MessageDefPtr message,
                                         const DefPoolPair& pools,
                                         Output& output) const {
        output("/* $0 */\n\n", message.full_name());
        std::string msg_name = ToCIdent(message.full_name());
        if (!HPB_DESC(MessageOptions_map_entry)(message.options())) {
            GenerateMessageFunctionsInSource(message, output);
        }

        for (int i = 0; i < message.real_oneof_count(); i++) {
            GenerateOneofDefine(message.oneof(i), pools, msg_name, output);
        }

        auto field_names = CreateFieldNameMap(message);
        for (auto field : FieldNumberOrder(message)) {
            GenerateClearDefine(field, pools, msg_name, field_names, output);
            GenerateGettersDefine(field, pools, msg_name, field_names, output);
            GenerateHazzerDefine(field, pools, msg_name, field_names, output);
        }

        output("\n");

        for (auto field : FieldNumberOrder(message)) {
            GenerateSettersDefine(field, pools, msg_name, field_names, output);
        }
    ...
    void HSChpb::GenerateMessageFunctionsInHeader(hpb::MessageDefPtr message,Output& output) const {
        output(
                R"cc(
        $0* $0_new(hpb_Arena* arena);

        $0* $0_parse(const char* buf, size_t size, hpb_Arena* arena);

        $0* $0_parse_ex(const char* buf, size_t size,
                                   const hpb_ExtensionRegistry* extreg,
                                   int options, hpb_Arena* arena);

        char* $0_serialize(const $0* msg, hpb_Arena* arena, size_t* len);

        char* $0_serialize_ex(const $0* msg, int options,
                                         hpb_Arena* arena, size_t* len);
      )cc",
                MessageName(message), MessageMiniTableRef(message));
    }

    void HSChpb::GenerateMessageFunctionsInSource(hpb::MessageDefPtr message,Output& output) const {
    ...
    std::string HSChpb::MessageMiniTableRef(hpb::MessageDefPtr descriptor) const {
        if (bootstrap) {
    ...
    void HSChpb::GenerateOneofDeclare(hpb::OneofDefPtr oneof, const DefPoolPair& pools, absl::string_view msg_name, Output& output) const {
        ...
        output(
                R"cc(
        $0_oneofcases $1_$2_case(const $1* msg);
      )cc",
                fullname, msg_name, oneof.name(),
                FieldInitializer(pools, oneof.field(0)));
    }

    void HSChpb::GenerateOneofDefine(hpb::OneofDefPtr oneof, const DefPoolPair& pools, absl::string_view msg_name, Output& output) const {
        std::string fullname = ToCIdent(oneof.full_name());
        output("typedef enum {\n");
        for (int j = 0; j < oneof.field_count(); j++) {
            hpb::FieldDefPtr field = oneof.field(j);
    ...
    std::string HSChpb::FieldInitializer(hpb::FieldDefPtr field,
    ...
    std::string HSChpb::FieldInitializer(const DefPoolPair& pools, hpb::FieldDefPtr field) const {
        return FieldInitializer(field, pools.GetField64(field),
                                pools.GetField32(field));
    }

    std::string HSChpb::ArchDependentSize(int64_t size32, int64_t size64) const {
        if (size32 == size64) return absl::StrCat(size32);
        return absl::Substitute("HPB_SIZE($0, $1)", size32, size64);
    }

    std::string HSChpb::GetModeInit(const hpb_MiniTableField* field32,
    ...
    std::string HSChpb::GetFieldRep(const hpb_MiniTableField* field32,
                            const hpb_MiniTableField* field64) const {
    ...
    void HSChpb::GenerateExtensionDeclare(const DefPoolPair& pools, hpb::FieldDefPtr ext,
                                           Output& output) const {
        output(
                R"cc(
        bool $0_has_$1(const struct $2* msg);
      )cc",
        ...
        if (ext.IsSequence()) {
          ...
          $0 $1_$2(const struct $3* msg);
            ...
            output(
                    R"cc(
          void $1_set_$2(struct $3* msg, $0 val, hpb_Arena* arena);
        )cc",
                    CTypeConst(ext), ExtensionIdentBase(ext), ext.name(),
                    MessageName(ext.containing_type()), ExtensionLayout(ext),
    ...
    void HSChpb::GenerateExtensionDefine(const DefPoolPair& pools, hpb::FieldDefPtr ext,
                                   Output& output) const {
        output(
                R"cc(
        bool $0_has_$1(const struct $2* msg) {
    ...
    std::string HSChpb::CTypeConst(hpb::FieldDefPtr field) const {
        return CTypeInternal(field, true);
    }

    std::string HSChpb::CTypeInternal(hpb::FieldDefPtr field, bool is_const) const {
    ...
    std::string HSChpb::FieldDefault(hpb::FieldDefPtr field) const {
        switch (field.ctype()) {
            case kHpb_CType_Message:
                return "NULL";
            case kHpb_CType_Bytes:
    ...
    std::string HSChpb::FloatToCLiteral(float value) const {
    ...
    std::string HSChpb::DoubleToCLiteral(double value) const {
    ...
    void HSChpb::GenerateClearDeclare(hpb::FieldDefPtr field, const DefPoolPair& pools,absl::string_view msg_name,
        ...
        output(
                R"cc(
        void $0_clear_$1($0* msg);
      )cc",
                msg_name, resolved_name, FieldInitializer(pools, field));
    }

    void HSChpb::GenerateClearDefine(hpb::FieldDefPtr field, const DefPoolPair& pools,absl::string_view msg_name,
                                      const NameToFieldDefMap& field_names,
                                      Output& output) const {
        if (field == field.containing_type().map_key() ||
            field == field.containing_type().map_value()) {
    ...
    void HSChpb::GenerateGettersDeclare(hpb::FieldDefPtr field, const DefPoolPair& pools,
                         absl::string_view msg_name,
                         const NameToFieldDefMap& field_names,Output& output) const {
        if (field.IsMap()) {
            GenerateMapGettersDeclare(field, pools, msg_name, field_names, output);
        } else if (HPB_DESC(MessageOptions_map_entry)(
                field.containing_type().options())) {
            GenerateMapEntryGettersDeclare(field, msg_name, output);
        } else if (field.IsSequence()) {
            GenerateRepeatedGettersDeclare(field, pools, msg_name, field_names,output);
        } else {
            GenerateScalarGettersDeclare(field, pools, msg_name, field_names, output);
        }
    }

    void HSChpb::GenerateGettersDefine(hpb::FieldDefPtr field, const DefPoolPair& pools,
                                        absl::string_view msg_name,
                                        const NameToFieldDefMap& field_names,Output& output) const {
        if (field.IsMap()) {
            GenerateMapGettersDefine(field, pools, msg_name, field_names, output);
        } else if (HPB_DESC(MessageOptions_map_entry)(
                field.containing_type().options())) {
            GenerateMapEntryGettersDefine(field, msg_name, output);
        } else if (field.IsSequence()) {
            GenerateRepeatedGettersDefine(field, pools, msg_name, field_names,output);
        } else {
            GenerateScalarGettersDefine(field, pools, msg_name, field_names, output);
        }
    }

    void HSChpb::GenerateMapGettersDeclare(hpb::FieldDefPtr field, const DefPoolPair& pools,
                            absl::string_view msg_name,
                            const NameToFieldDefMap& field_names, Output& output) const {
        std::string resolved_name = ResolveFieldName(field, field_names);
        output(
                R"cc(
        size_t $0_$1_size(const $0* msg);
      )cc",
                msg_name, resolved_name, FieldInitializer(pools, field));
        output(
                R"cc(
        bool $0_$1_get(const $0* msg, $2 key, $3* val);
      )cc",
        ...
        output(
                R"cc(
        $0 $1_$2_next(const $1* msg, size_t* iter);
      )cc",
                CTypeConst(field), msg_name, resolved_name,
                FieldInitializer(pools, field));
    }

    void HSChpb::GenerateMapGettersDefine(hpb::FieldDefPtr field, const DefPoolPair& pools,
                                           absl::string_view msg_name,
                                           const NameToFieldDefMap& field_names, Output& output) const {
        std::string resolved_name = ResolveFieldName(field, field_names);
        output(
    ...
    std::string HSChpb::CType(hpb::FieldDefPtr field) const {
        return CTypeInternal(field, false);
    }


    std::string HSChpb::MapKeyCType(hpb::FieldDefPtr map_field) const {
        return CType(map_field.message_type().map_key());
    }


    std::string HSChpb::MapValueCType(hpb::FieldDefPtr map_field) const {
        return CType(map_field.message_type().map_value());
    }

    std::string HSChpb::MapKeySize(hpb::FieldDefPtr map_field, absl::string_view expr) const {
        return map_field.message_type().map_key().ctype() == kHpb_CType_String
               ? "0"
               : absl::StrCat("sizeof(", expr, ")");
    }
    std::string HSChpb::MapValueSize(hpb::FieldDefPtr map_field, absl::string_view expr) const {
        return map_field.message_type().map_value().ctype() == kHpb_CType_String
               ? "0"
               : absl::StrCat("sizeof(", expr, ")");
    }

    void HSChpb::GenerateMapEntryGettersDeclare(hpb::FieldDefPtr field, absl::string_view msg_name,
                                 Output& output) const {
        output(
                R"cc(
        $0 $1_$2(const $1* msg);
    ...
    void HSChpb::GenerateMapEntryGettersDefine(hpb::FieldDefPtr field, absl::string_view msg_name,
                                         Output& output) const {
        output(
                R"cc(
        $0 $1_$2(const $1* msg) {
    ...
    void HSChpb::GenerateRepeatedGettersDeclare(hpb::FieldDefPtr field, const DefPoolPair& pools,
                                 absl::string_view msg_name,
                                 const NameToFieldDefMap& field_names, Output& output) const {
        // Generate getter returning first item and size.
        //
        // Example:
        //   const struct Bar* const* name(const Foo* msg, size_t* size)
        output(
                R"cc(
        $0 const* $1_$2(const $1* msg, size_t* size);
      )cc",
                CTypeConst(field),                       // $0
                msg_name,                                // $1
        ...
        //
        // Example:
        //   const hpb_Array* _name_hpbarray(size_t* size)
        //   hpb_Array* _name_mutable_hpbarray(size_t* size)
        output(
                R"cc(
        const hpb_Array* _$1_$2_$4(const $1* msg, size_t* size);

        hpb_Array* _$1_$2_$5(const $1* msg, size_t* size, hpb_Arena* arena);
      )cc",
                CTypeConst(field),                        // $0
                msg_name,                                 // $1
                ResolveFieldName(field, field_names),     // $2
    ...
    void HSChpb::GenerateRepeatedGettersDefine(hpb::FieldDefPtr field, const DefPoolPair& pools,
                                         absl::string_view msg_name,
                                         const NameToFieldDefMap& field_names, Output& output) const {
        // Generate getter returning first item and size.
        //
    ...
    void HSChpb::GenerateScalarGettersDeclare(hpb::FieldDefPtr field, const DefPoolPair& pools,
                               absl::string_view msg_name,
                               const NameToFieldDefMap& field_names,Output& output) const {
    ...
    void HSChpb::GenerateScalarGettersDefine(hpb::FieldDefPtr field, const DefPoolPair& pools,
                                       absl::string_view msg_name,
                                       const NameToFieldDefMap& field_names,Output& output) const {
        std::string field_name = ResolveFieldName(field, field_names);
        output(
    ...
    void HSChpb::GenerateHazzerDeclare(hpb::FieldDefPtr field, const DefPoolPair& pools,
                        absl::string_view msg_name,
                        const NameToFieldDefMap& field_names, Output& output) const {
        std::string resolved_name = ResolveFieldName(field, field_names);
    ...
    void HSChpb::GenerateHazzerDefine(hpb::FieldDefPtr field, const DefPoolPair& pools,
                                absl::string_view msg_name,
                                const NameToFieldDefMap& field_names, Output& output) const {
        std::string resolved_name = ResolveFieldName(field, field_names);
        if (field.has_presence()) {
    ...
    void HSChpb::GenerateSettersDefine(hpb::FieldDefPtr field, const DefPoolPair& pools,
                         absl::string_view msg_name,
                         const NameToFieldDefMap& field_names,Output& output) const {
        if (field.IsMap()) {
            GenerateMapSettersDefine(field, pools, msg_name, field_names, output);
        } else if (field.IsSequence()) {
            GenerateRepeatedSettersDefine(field, pools, msg_name, field_names,
                                    output);
        } else {
            GenerateNonRepeatedSettersDefine(field, pools, msg_name, field_names,output);
        }
    }

    void HSChpb::GenerateSettersDeclare(hpb::FieldDefPtr field, const DefPoolPair& pools,
                                 absl::string_view msg_name,
                                 const NameToFieldDefMap& field_names,Output& output) const {
        if (field.IsMap()) {
            GenerateMapSettersDeclare(field, pools, msg_name, field_names, output);
        } else if (field.IsSequence()) {
            GenerateRepeatedSettersDeclare(field, pools, msg_name, field_names,
                                    output);
        } else {
            GenerateNonRepeatedSettersDeclare(field, pools, msg_name, field_names,output);
        }
    }


    void HSChpb::GenerateMapSettersDeclare(hpb::FieldDefPtr field, const DefPoolPair& pools,
                            absl::string_view msg_name,
                            const NameToFieldDefMap& field_names,Output& output) const {
        std::string resolved_name = ResolveFieldName(field, field_names);
        output(
                R"cc(
        void $0_$1_clear($0* msg);
      )cc",
                msg_name, resolved_name, FieldInitializer(pools, field));
        output(
                R"cc(
        bool $0_$1_set($0* msg, $2 key, $3 val, hpb_Arena* a);
      )cc",
                msg_name, resolved_name, MapKeyCType(field), MapValueCType(field),
                FieldInitializer(pools, field), MapKeySize(field, "key"),
                MapValueSize(field, "val"));
        output(
                R"cc(
        bool $0_$1_delete($0* msg, $2 key);
      )cc",
                msg_name, resolved_name, MapKeyCType(field),
                FieldInitializer(pools, field), MapKeySize(field, "key"));
        output(
                R"cc(
        $0 $1_$2_nextmutable($1* msg, size_t* iter);
      )cc",
                CType(field), msg_name, resolved_name,
                FieldInitializer(pools, field));
    }

    void HSChpb::GenerateMapSettersDefine(hpb::FieldDefPtr field, const DefPoolPair& pools,
                                    absl::string_view msg_name,
                                    const NameToFieldDefMap& field_names,Output& output) const {
        std::string resolved_name = ResolveFieldName(field, field_names);
        output(
    ...
    void HSChpb::GenerateRepeatedSettersDeclare(hpb::FieldDefPtr field, const DefPoolPair& pools,
                                 absl::string_view msg_name,
                                 const NameToFieldDefMap& field_names,Output& output) const {
        std::string resolved_name = ResolveFieldName(field, field_names);
        output(
                R"cc(
        $0* $1_mutable_$2($1* msg, size_t* size);
      )cc",
                CType(field), msg_name, resolved_name,
                FieldInitializer(pools, field));
        output(
                R"cc(
        $0* $1_resize_$2($1* msg, size_t size, hpb_Arena* arena);
      )cc",
                CType(field), msg_name, resolved_name,
                FieldInitializer(pools, field));
        if (field.ctype() == kHpb_CType_Message) {
            output(
                    R"cc(
          struct $0* $1_add_$2($1* msg, hpb_Arena* arena);
        )cc",
            ...
            output(
                    R"cc(
          bool $1_add_$2($1* msg, $0 val, hpb_Arena* arena);
        )cc",
                    CType(field), msg_name, resolved_name,
                    FieldInitializer(pools, field));
        }
    }
    void HSChpb::GenerateRepeatedSettersDefine(hpb::FieldDefPtr field, const DefPoolPair& pools,
                                         absl::string_view msg_name,
                                         const NameToFieldDefMap& field_names,Output& output) const {
        std::string resolved_name = ResolveFieldName(field, field_names);
        output(
    ...
    void HSChpb::GenerateNonRepeatedSettersDeclare(hpb::FieldDefPtr field,
                                    const DefPoolPair& pools,
                                    absl::string_view msg_name,
                                    const NameToFieldDefMap& field_names,Output& output) const {
        if (field == field.containing_type().map_key()) {
        ...
        if (field.ctype() == kHpb_CType_Message &&
            ...
            output(
                    R"cc(
          struct $0* $1_mutable_$2($1* msg, hpb_Arena* arena);
        )cc",
                    MessageName(field.message_type()), msg_name, field_name,
                    MessageMiniTableRef(field.message_type()));
        }
    }
    void HSChpb::GenerateNonRepeatedSettersDefine(hpb::FieldDefPtr field,
                                            const DefPoolPair& pools,
                                            absl::string_view msg_name,
                                            const NameToFieldDefMap& field_names,Output& output) const {
        if (field == field.containing_type().map_key()) {
    ...
    void HSChpb::WriteSource(const DefPoolPair& pools, hpb::FileDefPtr file, Output& output) const {
        if (bootstrap) {
        ...
        const std::vector<hpb::FieldDefPtr> this_file_exts = SortedExtensions(file);
        for (auto message : this_file_messages) {
            GenerateMessageInSource(message, pools, output);
        }

        for (auto ext : this_file_exts) {
            GenerateExtensionDefine(pools, ext, output);
        }
    }


    void HSChpb::WriteMiniDescriptorSource(const DefPoolPair& pools, hpb::FileDefPtr file,Output& output) const {
        output(
                "#include <stddef.h>\n"
                "#include \"hpb/generated_code_support.h\"\n"
                "#include \"$0\"\n\n",
    ...
    void HSChpb::WriteMessageMiniDescriptorInitializer(hpb::MessageDefPtr msg,
                                               Output& output) const {
    ...
    std::string HSChpb::EnumMiniTableRef(hpb::EnumDefPtr descriptor) const {
        if (bootstrap) {
    ...
    std::string HSChpb::EnumInitName(hpb::EnumDefPtr descriptor) const {
        return ToCIdent(descriptor.full_name()) + "_enum_init";
    }

    void HSChpb::WriteEnumMiniDescriptorInitializer(hpb::EnumDefPtr enum_def,Output& output) const {
        output(
                R"cc(
    ...
    void HSChpb::WriteMiniTableSource(const DefPoolPair& pools, hpb::FileDefPtr file,Output& output) const {
        EmitFileWarning(file.name(), output);
    ...
    int HSChpb::WriteMessages(const DefPoolPair& pools, hpb::FileDefPtr file,Output& output) const {
    ...
    void HSChpb::WriteMessage(hpb::MessageDefPtr message, const DefPoolPair& pools, Output& output) const {
        std::string msg_name = ToCIdent(message.full_name());
    ...
    std::string HSChpb::GetSub(hpb::FieldDefPtr field) const {
    ...
    int HSChpb::WriteExtensions(const DefPoolPair& pools, hpb::FileDefPtr file, Output& output) const {
    ...
    void HSChpb::WriteExtension(hpb::FieldDefPtr ext, const DefPoolPair& pools,Output& output) const {
        output("$0,\n", FieldInitializer(pools, ext));
        output("  &$0,\n", MessageInitName(ext.containing_type()));
        output("  $0,\n", GetSub(ext));
    }

    int HSChpb::WriteEnums(const DefPoolPair& pools, hpb::FileDefPtr file, Output& output) const {
    ...
    void HSChpb::WriteEnum(hpb::EnumDefPtr e, Output& output) const {
    ...
    void HSChpb::WriteMessageField(hpb::FieldDefPtr field,
                           const hpb_MiniTableField* field64,
                           const hpb_MiniTableField* field32,Output& output) const {
        output("  $0,\n", FieldInitializer(field, field64, field32));
    }

    std::vector<TableEntry> HSChpb::FastDecodeTable(hpb::MessageDefPtr message,
    ...
    std::vector<hpb::FieldDefPtr> HSChpb::FieldHotnessOrder(hpb::MessageDefPtr message) const {
    ...
    int HSChpb::GetTableSlot(hpb::FieldDefPtr field) const {
    ...
    uint64_t HSChpb::GetEncodedTag(hpb::FieldDefPtr field) const {
    ...
    uint32_t HSChpb::GetWireTypeForField(hpb::FieldDefPtr field) const {
    ...
    bool HSChpb::TryFillTableEntry(const DefPoolPair& pools, hpb::FieldDefPtr field, TableEntry& ent) const {
