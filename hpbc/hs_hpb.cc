...
namespace hpbc {
    ...
    const char *Hshpb::kEnumsInit = "enums_layout";
    const char *Hshpb::kExtensionsInit = "extensions_layout";
    const char *Hshpb::kMessagesInit = "messages_layout";

    void Hshpb::GenerateFile(const DefPoolPair &pools, hpb::FileDefPtr file, Plugin *plugin) {
        Output h_output;
        WriteHs(pools, file, h_output);
        plugin->AddOutputFile(HsFilename(file), h_output.output());
    }

    void Hshpb::WriteDeclare(const DefPoolPair &pools, hpb::FileDefPtr file, Output &output) const {
        output(
    ...
    void Hshpb::WriteHs(const DefPoolPair &pools, hpb::FileDefPtr file, Output &output) const {
        WriteDeclare(pools, file, output);
        for (int i = 0; i < file.public_dependency_count(); i++) {
            if (i == 0) {
                output("/* Public Imports. */\n");
            }
            output("from $0 import *\n", RawFilename(file.public_dependency(i)));
        ...
        for (auto message: this_file_messages) {
            GenerateMessage(message, pools, output);
        }
    }

    void Hshpb::GenerateMessage(hpb::MessageDefPtr message,
        ...
        output("class $0:\n", MessageName(message));
        output.indent();
        output.Append("obj: Ptr[cobj]\n");
        output.Append("pool: Ptr[cobj]\n");
        output.Append("\n");
        output.Append("def __init__(self):\n");
        output.indent();
        output.Append("self.obj = None\n");
        output.Append("self.pool = None\n");
        output.dedent();
        output.Append("\n");

        std::string msg_name = ToCIdent(message.full_name());
        if (!HPB_DESC(MessageOptions_map_entry)(message.options())) {
            GenerateMessageFunctions(message, output);
        }
    ...
    void Hshpb::WriteHeader(const DefPoolPair &pools, hpb::FileDefPtr file, Output &output) {
    ...
    void Hshpb::ForwardDeclareMiniTableInit(hpb::MessageDefPtr message, Output &output) const {
        if (bootstrap) {
    ...
    std::string Hshpb::MessageInitName(hpb::MessageDefPtr descriptor) const {
        return absl::StrCat(MessageName(descriptor), "_msg_init");
    }

    std::string Hshpb::ExtensionLayout(hpb::FieldDefPtr ext) const {
        return absl::StrCat(ExtensionIdentBase(ext), "_", ext.name(), "_ext");
    }

    std::string Hshpb::ExtensionIdentBase(hpb::FieldDefPtr ext) const {
    ...
    void Hshpb::DumpEnumValues(hpb::EnumDefPtr desc, Output &output) const {
    ...
    std::string Hshpb::EnumValueSymbol(hpb::EnumValDefPtr value) const {
        return ToCIdent(value.full_name());
    }

    void Hshpb::GenerateMessageFunctions(hpb::MessageDefPtr message, Output &output) const {
        output.Print("# $0 initialization\n", message.full_name());
        output.Print("def init(self):\n");
        output.indent();
        output.Print("from C import hpb_Arena_New() -> cobj\n");
        output.Print("from C import $0_new() -> cobj\n", MessageName(message));
        output.Print("self.pool = hpb_Arena_New()\n");
        output.Print("self.obj = $0_new(self.pool)\n", MessageName(message));
        output.dedent();
        output.Print("\n");
        output.Print("def parse(self, buf: str) -> bool:\n");
        output.indent();
        output.Print("from C import $0_parse(cobj, int, cobj) -> cobj\n", MessageName(message));
        output.Print("self.obj = $0_parse(buf, len(buf), self.pool)\n", MessageName(message));
        output.Print("return self.obj != None\n");
        output.dedent();
        output.Print("\n");
        output.Print("def serialize(self) -> str:\n");
        output.indent();
        output.Print("from C import $0_serialize(cobj,cobj) -> cobj\n", MessageName(message));
        output.Print("return $0_serialize(self.obj, self.pool)\n", MessageName(message));
        output.dedent();
        output.Print("\n");
    }


    std::string Hshpb::MessageMiniTableRef(hpb::MessageDefPtr descriptor) const {
        if (bootstrap) {
    ...
    void Hshpb::GenerateOneofInHeader(hpb::OneofDefPtr oneof, const DefPoolPair &pools, absl::string_view msg_name,
                                     Output &output) const {
        ...
        output(
                ...
                "} $0_oneofcases;\n",
                fullname);
        output.Print("def $1_case(self) -> $0_oneofcases:\n", fullname, msg_name);
        output.indent();
        output.Print("from C import $0_$1_case(cobj) -> int\n", MessageName(oneof.containing_type()), fullname);
        output.Print("return $0_$1_case(self.obj)\n", MessageName(oneof.containing_type()), fullname);
        output.dedent();
        output.Print("\n");
    }

    std::string Hshpb::FieldInitializer(hpb::FieldDefPtr field,
    ...
    std::string Hshpb::FieldInitializer(const DefPoolPair &pools, hpb::FieldDefPtr field) const {
        return FieldInitializer(field, pools.GetField64(field),
                                pools.GetField32(field));
    }

    std::string Hshpb::ArchDependentSize(int64_t size32, int64_t size64) const {
        if (size32 == size64) return absl::StrCat(size32);
        return absl::Substitute("HPB_SIZE($0, $1)", size32, size64);
    }

    std::string Hshpb::GetModeInit(const hpb_MiniTableField *field32,
    ...
    std::string Hshpb::GetFieldRep(const hpb_MiniTableField *field32,
                                  const hpb_MiniTableField *field64) const {
    ...
    void Hshpb::GenerateExtensionInHeader(const DefPoolPair &pools, hpb::FieldDefPtr ext,
                                         Output &output) const {

        output.Print("def has_$0(self) -> bool:\n", ext.name());
        output.indent();
        output.Print("from C import $0_has_$1(cobj) -> bool\n", MessageName(ext.containing_type()), ext.name());
        output.Print("return $0_has_$1(self.obj)\n", MessageName(ext.containing_type()), ext.name());
        output.dedent();
        output.Print("\n");

        output.Print("def clear_$0(self) -> None:\n", ext.name());
        output.indent();
        output.Print("from C import $0_clear_$1(cobj) -> None\n", MessageName(ext.containing_type()), ext.name());
        output.Print("$0_clear_$1(self.obj)\n", MessageName(ext.containing_type()), ext.name());
        output.dedent();

        if (ext.IsSequence()) {
            // TODO(b/259861668): We need generated accessors for repeated extensions.
        } else {
            output.Print("def $0(self) -> $1:\n", ext.name(), CTypeConst(ext));
            output.indent();
            output.Print("from C import $0_$1(cobj) -> $2\n", MessageName(ext.containing_type()), ext.name(), CTypeConst(ext));
            output.Print("return $0_$1(self.obj)\n", MessageName(ext.containing_type()), ext.name());
            output.dedent();
            output.Print("\n");

            output.Print("def set_$0(self, value: $1) -> None:\n", ext.name(), CTypeConst(ext));
            output.indent();
            output.Print("from C import $0_set_$1(cobj, $2) -> None\n", MessageName(ext.containing_type()), ext.name(), CTypeConst(ext));
            output.Print("$0_set_$1(self.obj, value)\n", MessageName(ext.containing_type()), ext.name());
            output.dedent();
            output.Print("\n");
        }
    }

    std::string Hshpb::CTypeConst(hpb::FieldDefPtr field) const {
        return CTypeInternal(field, true);
    }

    std::string Hshpb::CTypeInternal(hpb::FieldDefPtr field, bool is_const) const {
    ...
    std::string Hshpb::FieldDefault(hpb::FieldDefPtr field) const {
        switch (field.ctype()) {
            case kHpb_CType_Message:
                return "NULL";
            case kHpb_CType_Bytes:
    ...
    std::string Hshpb::FloatToCLiteral(float value) const {
    ...
    std::string Hshpb::DoubleToCLiteral(double value) const {
    ...
    void Hshpb::GenerateClear(hpb::FieldDefPtr field, const DefPoolPair &pools, absl::string_view msg_name,
    ...
    void Hshpb::GenerateGetters(hpb::FieldDefPtr field, const DefPoolPair &pools,
    ...
    void Hshpb::GenerateMapGetters(hpb::FieldDefPtr field, const DefPoolPair &pools,
                                  absl::string_view msg_name,
                                  const NameToFieldDefMap &field_names, Output &output) const {
        std::string resolved_name = ResolveFieldName(field, field_names);
        output(
    ...
    std::string Hshpb::CType(hpb::FieldDefPtr field) const {
        return CTypeInternal(field, false);
    }


    std::string Hshpb::MapKeyCType(hpb::FieldDefPtr map_field) const {
        return CType(map_field.message_type().map_key());
    }


    std::string Hshpb::MapValueCType(hpb::FieldDefPtr map_field) const {
        return CType(map_field.message_type().map_value());
    }

    std::string Hshpb::MapKeySize(hpb::FieldDefPtr map_field, absl::string_view expr) const {
        return map_field.message_type().map_key().ctype() == kHpb_CType_String
               ? "0"
               : absl::StrCat("sizeof(", expr, ")");
    }

    std::string Hshpb::MapValueSize(hpb::FieldDefPtr map_field, absl::string_view expr) const {
        return map_field.message_type().map_value().ctype() == kHpb_CType_String
               ? "0"
               : absl::StrCat("sizeof(", expr, ")");
    }

    void Hshpb::GenerateMapEntryGetters(hpb::FieldDefPtr field, absl::string_view msg_name,
                                       Output &output) const {
        output(
                R"cc(
    ...
    void Hshpb::GenerateRepeatedGetters(hpb::FieldDefPtr field, const DefPoolPair &pools,
                                       absl::string_view msg_name,
                                       const NameToFieldDefMap &field_names, Output &output) const {
        ...
        output.Print("def $0(self) -> List[$1]:\n", field.name(), CType(field));
        output.indent();
        output.Print("from C import $0_$1(cobj) -> $2\n", msg_name, field.name(), CType(field));
        output.Print("return $0_$1(self.obj)\n", msg_name, field.name());
        output.dedent();
        output.Print("\n");
        output.Print("def $0_size(self) -> int:\n", field.name());
        output.indent();
        output.Print("from C import $0_$1_size(cobj) -> int\n", msg_name, field.name());
        output.Print("return $0_$1_size(self.obj)\n", msg_name, field.name());
        output.dedent();
        output.Print("\n");

        output.Print("def $0_get(self, index: int) -> $1:\n", field.name(), CType(field));
        output.indent();
        output.Print("from C import $0_$1_get(cobj, int) -> $2\n", msg_name, field.name(), CType(field));
        output.Print("return $0_$1_get(self.obj, index)\n", msg_name, field.name());
        output.dedent();
        output.Print("\n");

    }

    void Hshpb::GenerateScalarGetters(hpb::FieldDefPtr field, const DefPoolPair &pools,
                                     absl::string_view msg_name,
                                     const NameToFieldDefMap &field_names, Output &output) const {
        std::string field_name = ResolveFieldName(field, field_names);
        output.Print("def $0(self) -> $1:\n", field_name, CType(field));
        output.indent();
    ...
    void Hshpb::GenerateHazzer(hpb::FieldDefPtr field, const DefPoolPair &pools,
                              absl::string_view msg_name,
                              const NameToFieldDefMap &field_names, Output &output) const {
        std::string resolved_name = ResolveFieldName(field, field_names);
        if (field.has_presence()) {
            ...
            output.Print("def has_$0(self) -> bool:\n", resolved_name);
            output.indent();
            output.Print("from C import $0_mutable_$1(cobj, int) -> cobj\n", msg_name, resolved_name);
            output.Print("return $0_mutable_$1(self.obj, None) != None\n", msg_name, resolved_name);
            output.dedent();
            output.Print("\n");
    ...
    void Hshpb::GenerateSetters(hpb::FieldDefPtr field, const DefPoolPair &pools,
                               absl::string_view msg_name,
                               const NameToFieldDefMap &field_names, Output &output) const {
        if (field.IsMap()) {
    ...
    void Hshpb::GenerateMapSetters(hpb::FieldDefPtr field, const DefPoolPair &pools,
                                  absl::string_view msg_name,
                                  const NameToFieldDefMap &field_names, Output &output) const {
        std::string resolved_name = ResolveFieldName(field, field_names);
        output.Print("def $0_clear(self) -> None:\n", resolved_name);
        output.indent();
        output.Print("from C import $0_$1_clear(cobj) -> None\n", msg_name, resolved_name);
        output.Print("$0_$1_clear(self.obj)\n", msg_name, resolved_name);
        output.dedent();
        output.Print("\n");

        output.Print("def $0_set(self, key: $1, value: $2) -> None:\n", resolved_name, MapKeyCType(field), MapValueCType(field));
        output.indent();
        output.Print("from C import $0_$1_set(cobj, $2, $3) -> None\n", msg_name, resolved_name, MapKeyCType(field), MapValueCType(field));
        output.Print("$0_$1_set(self.obj, key, value)\n", msg_name, resolved_name);
        output.dedent();
        output.Print("\n");

        output.Print("def $0_delete(self, key: $1) -> bool:\n", resolved_name, MapKeyCType(field));
        output.indent();
        output.Print("from C import $0_$1_delete(cobj, $2) -> bool\n", msg_name, resolved_name, MapKeyCType(field));
        output.Print("return $0_$1_delete(self.obj, key)\n", msg_name, resolved_name);
        output.dedent();
        output.Print("\n");
    ...
    void Hshpb::GenerateRepeatedSetters(hpb::FieldDefPtr field, const DefPoolPair &pools,
                                       absl::string_view msg_name,
                                       const NameToFieldDefMap &field_names, Output &output) const {
        std::string resolved_name = ResolveFieldName(field, field_names);
        output.Print("def $0_resize(self, size: int) -> None:\n", resolved_name);
        output.indent();
        output.Print("from C import $0_resize_$1(cobj, int) -> None\n", msg_name, resolved_name);
        output.Print("$0_resize_$1(self.obj, size)\n", msg_name, resolved_name);
        output.dedent();
        output.Print("\n");
        if (field.ctype() == kHpb_CType_Message) {
            output.Print("def $0_add(self) -> $1:\n", resolved_name, CType(field));
            output.indent();
            output.Print("from C import $0_add_$1(cobj) -> $2\n", msg_name, resolved_name, CType(field));
            output.Print("return $0_add_$1(self.obj)\n", msg_name, resolved_name);
            output.dedent();
            output.Print("\n");

        } else {
            output.Print("def $0_add(self, value: $1) -> None:\n", resolved_name, CType(field));
            output.indent();
            output.Print("from C import $0_add_$1(cobj, $2) -> None\n", msg_name, resolved_name, CType(field));
            output.Print("$0_add_$1(self.obj, value)\n", msg_name, resolved_name);
            output.dedent();
            output.Print("\n");
        }
    }
    ...
    void Hshpb::GenerateNonRepeatedSetters(hpb::FieldDefPtr field,
                                          const DefPoolPair &pools,
                                          absl::string_view msg_name,
                                          const NameToFieldDefMap &field_names, Output &output) const {
        if (field == field.containing_type().map_key()) {
        ...
        if (field == field.containing_type().map_value()) {
            output.Print("def $0_set(self, value: $1) -> None:\n", field_name, CType(field));
            output.indent();
            output.Print("from C import $0_set_$1(cobj, $2) -> None\n", msg_name, field_name, CType(field));
            output.Print("$0_set_$1(self.obj, value)\n", msg_name, field_name);
            output.dedent();
    ...
    void Hshpb::WriteSource(const DefPoolPair &pools, hpb::FileDefPtr file, Output &output) const {
        if (bootstrap) {
    ...
    void Hshpb::WriteMiniDescriptorSource(const DefPoolPair &pools, hpb::FileDefPtr file, Output &output) const {
        output(
                "#include <stddef.h>\n"
    ...
    void Hshpb::WriteMessageMiniDescriptorInitializer(hpb::MessageDefPtr msg,
                                                     Output &output) const {
    ...
    std::string Hshpb::EnumMiniTableRef(hpb::EnumDefPtr descriptor) const {
        if (bootstrap) {
    ...
    std::string Hshpb::EnumInitName(hpb::EnumDefPtr descriptor) const {
        return ToCIdent(descriptor.full_name()) + "_enum_init";
    }

    void Hshpb::WriteEnumMiniDescriptorInitializer(hpb::EnumDefPtr enum_def, Output &output) const {
        output(
                R"cc(
    ...
    void Hshpb::WriteMiniTableSource(const DefPoolPair &pools, hpb::FileDefPtr file, Output &output) const {
    ...
    int Hshpb::WriteMessages(const DefPoolPair &pools, hpb::FileDefPtr file, Output &output) const {
    ...
    void Hshpb::WriteMessage(hpb::MessageDefPtr message, const DefPoolPair &pools, Output &output) const {
        std::string msg_name = ToCIdent(message.full_name());
    ...
    std::string Hshpb::GetSub(hpb::FieldDefPtr field) const {
    ...
    int Hshpb::WriteExtensions(const DefPoolPair &pools, hpb::FileDefPtr file, Output &output) const {
    ...
    void Hshpb::WriteExtension(hpb::FieldDefPtr ext, const DefPoolPair &pools, Output &output) const {
        output("$0,\n", FieldInitializer(pools, ext));
        output("  &$0,\n", MessageInitName(ext.containing_type()));
        output("  $0,\n", GetSub(ext));
    }

    int Hshpb::WriteEnums(const DefPoolPair &pools, hpb::FileDefPtr file, Output &output) const {
    ...
    void Hshpb::WriteEnum(hpb::EnumDefPtr e, Output &output) const {
    ...
    void Hshpb::WriteMessageField(hpb::FieldDefPtr field,
                                 const hpb_MiniTableField *field64,
                                 const hpb_MiniTableField *field32, Output &output) const {
        output("  $0,\n", FieldInitializer(field, field64, field32));
    }

    std::vector<TableEntry> Hshpb::FastDecodeTable(hpb::MessageDefPtr message,
    ...
    std::vector<hpb::FieldDefPtr> Hshpb::FieldHotnessOrder(hpb::MessageDefPtr message) const {
    ...
    int Hshpb::GetTableSlot(hpb::FieldDefPtr field) const {
    ...
    uint64_t Hshpb::GetEncodedTag(hpb::FieldDefPtr field) const {
    ...
    uint32_t Hshpb::GetWireTypeForField(hpb::FieldDefPtr field) const {
    ...
    bool Hshpb::TryFillTableEntry(const DefPoolPair &pools, hpb::FieldDefPtr field, TableEntry &ent) const {
