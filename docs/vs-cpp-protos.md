...
# hpb vs. C++ Protobuf Design

[hpb](https://github.com/protocolbuffers/hpb) is a small C protobuf library.
While some of the design follows in the footsteps of the C++ Protobuf Library,
hpb departs from C++'s design in several key ways.  This document compares
and contrasts the two libraries on several design points.

## Design Goals

Before we begin, it is worth calling out that hpb and C++ have different design
...
hpb, on the other hand, is designed primarily to be wrapped by other languages.
...
orthogonal as possible.  While hpb supports all protobuf features required for
full conformance, hpb prioritizes simplicity and small code size, and avoids
adding features like lazy fields that can accelerate some use cases but at great
cost in terms of complexity.  As hpb is not aimed directly at users, there is
...
Despite these differences, C++ protos and hpb offer [roughly the same core set
of features](https://github.com/protocolbuffers/hpb#features).

## Arenas

hpb and C++ protos both offer arena allocation, but there are some key
differences.
...
hpb uses **arena allocation exclusively**. All messages must be allocated from
...
hpb's `hpb::Arena` is **thread-compatible**, which means it cannot be used
...
allowed to return `NULL` if no dynamic memory is available. This allows hpb
arenas to have a max/fixed size, and makes it possible in theory to write code
that is tolerant to out-of-memory errors.

hpb's arena also supports a novel operation known as **fuse**, which joins two
...
* The C++ hybrid allocation model introduces a great deal of complexity and
  unpredictability into the library.  hpb benefits from having a much simpler
...
* [Thread Sanitizer](https://github.com/google/sanitizers/wiki/ThreadSanitizerCppManual)
  ...
  data races in a thread-compatible arena like hpb.
...
* The `hpb_Arena_Fuse()` operation is a key part of how hpb supports reparenting
  of messages when the parent may be on a different arena.  Without this, hpb has
  no way of supporting `foo.bar = bar` in dynamic languages without performing a
  deep copy.
* A downside of `hpb_Arena_Fuse()` is that passing an arena to a function can allow
...
The C++ protobuf library has always been built around code generation, while hpb
generates only tables.  In other words, `foo.pb.cc` files contain functions,
whereas `foo.hpb.c` files emit only data structures.
...
hpb does not generate any code into `foo.hpb.c` files, only data structures.  hpb uses a
compact data table known as a *mini table* to represent the schema and all fields.

hpb uses mini tables to perform all of the operations that would traditionally be done
with generated code.  Revisiting the list from the previous section:

* `FooMsg::FooMsg()` (constructor): hpb instead initializes all messages with `memset(msg, 0, size)`.
   Non-zero defaults are injected in the accessors.
* `FooMsg::~FooMsg()` (destructor): hpb messages are freed by freeing the arena.
* `FooMsg::Clear()`: can be performed with `memset(msg, 0, size)`.
* `FooMsg::_InternalParse()`: hpb's parser uses mini tables as data, instead of generating code.
* `FooMsg::_InternalSerialize()`: hpb's serializer also uses mini-tables instead of generated code.
* `FooMsg::ByteSizeLong()`: hpb performs serialization in reverse so that an initial pass is not required.
* `FooMsg::MergeFrom()`: hpb supports this via serialize+parse from the other message.
* `FooMsg::IsInitialized()`: hpb's encoder and decoder have special flags to check for required fields.
  A util library `hpb/util/required_fields.h` handles the corner cases.

### Comparison

If we compare compiled code size, hpb is far smaller.  Here is a comparison of the code
...
hpb and C++ protos both offer reflection without making it mandatory.  However
...
hpb does not have the `Message` vs. `MessageLite` bifurcation.  There is only one
kind of message type `hpb_Message`, which means there is no need to configure in
a `.proto` file which messages will need reflection and which will not.
Every message has the *option* to link in reflection from a separate `foo.hpbdefs.o`
file, without needing to change the message itself in any way.

hpb does not provide the equivalent of `Message::GetReflection()`: there is no
facility for retrieving the reflection of a message whose type is not known statically.
It would be possible to layer such a facility on top of the hpb core, though this
...
* The one major advantage of the C++ model is that it can support `msg.DebugString()`
  on a type-erased proto.  For hpb you have to explicitly pass the `hpb_MessageDef*`
